  ...
  if (p->wrapper != NULL) {
    ...
    if (is_last_wrapper) {
      if (in_persistent_list) {
        // If ref_count==0 and the key still in the list,
        // it means the user don't call channel->close().
        // persistent list should free the allocation in such case,
        // as well as related wrapped channel.
        if (p->wrapper->wrapped != NULL) {
          gpr_mu_lock(&p->wrapper->mu);
          grpc_channel_destroy(p->wrapper->wrapped);
          free(p->wrapper->target);
...
PHP_METHOD(Channel, close) {
  ...
  if (channel->wrapper != NULL) {
    ...
    if (channel->wrapper->wrapped != NULL) {
      if(channel->wrapper->is_valid) {
        // channel hasn't been destoryed by other wrapper.
  ...
  // set channel->wrapper to NULL to avoid call close twice for the same channel
